...
public class ResourceManager extends SWTResourceManager {
	...
	public static Font getWidgetFont(String fontKey, Control control, int style, int deltaSize){
		Font font = m_fontMap.get(fontKey);
		if (font == null || font.isDisposed()){
			FontData[] fontData = control.getFont().getFontData();
	        for (int i = 0; i < fontData.length; i++) {
	            fontData[i].setHeight(fontData[i].getHeight() + deltaSize);
	            fontData[i].setStyle(style);
	        }
	        font = new Font(control.getDisplay(), fontData);
	        m_fontMap.put(fontKey, font);
		}
		return font;
	}
	
	public static void disposeWidgetFonts(){
		// clear fonts
