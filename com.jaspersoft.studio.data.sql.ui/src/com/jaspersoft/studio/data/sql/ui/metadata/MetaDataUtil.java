...
public class MetaDataUtil {
	...
	public synchronized static void readSchema(DBMetadata dbmeta, DatabaseMetaData meta, MSqlSchema schema,
			IProgressMonitor monitor, List<String> tableTypes) {
		ResultSet rs = null;
		try {
			boolean isSchema = meta.supportsSchemasInTableDefinitions();
			boolean isCatalog = meta.supportsCatalogsInTableDefinitions();
			if (!isSchema && !isCatalog && schema.getValue() == null) {
				...
				while (rs.next()) {
					...
					if (tableSchema != null && tableSchema.equals(schema.getValue())) {
						schema.removeChildren();
						schema.setNotInMetadata(false);
...
//				}
	...
	public static void readTables(DBMetadata dbmeta, DatabaseMetaData meta, String tableSchema, String tableCatalog,
			MTables mview, LinkedHashMap<String, MSqlTable> tblMap, IProgressMonitor monitor) {
	...
	public synchronized static void readTableKeys(DatabaseMetaData meta, MSqlTable mtable, IProgressMonitor monitor)
			throws SQLException {
	...
	private static void readForeignKeys(DatabaseMetaData meta, MSqlTable mt, IProgressMonitor monitor)
			throws SQLException {
		MTables tables = (MTables) mt.getParent();
