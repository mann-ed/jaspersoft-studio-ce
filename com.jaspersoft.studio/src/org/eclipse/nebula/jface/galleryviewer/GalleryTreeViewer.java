/*
 * Copyright (c) 2007-2008 Peter Centgraf. All rights reserved. This program and the accompanying materials are made
 * available under the terms of the Eclipse Public License v1.0 which accompanies this distribution, and is available at
 * http://www.eclipse.org/legal/epl-v10.html
 * 
 * Contributors : Peter Centgraf - initial implementation
 * *****************************************************************************
 */
...
/*
 * A concrete viewer based on a Nebula <code>Gallery</code> control. <p> This class is not intended to be subclassed
 * outside the viewer framework. It is designed to be instantiated with a pre-existing Nebula Gallery control and
 * configured with a domain-specific content provider, label provider, element filter (optional), and element sorter
 * (optional). </p>
 * 
 * <p> SWT.VIRTUAL is currently unsupported </p>
 * 
 * <p> THIS VIEWER SHOULD BE CONSIDERED AS ALPHA </p>
 * 
...
public class GalleryTreeViewer extends AbstractTreeViewer {
	...
	/**
	 * Creates a gallery viewer on a newly-created gallery control under the given parent. The gallery control is created
	 * using the SWT style bits <code>MULTI, V_SCROLL,</code> and <code>BORDER</code>. The viewer has no input, no content
	...
	/**
	 * Creates a gallery viewer on a newly-created gallery control under the given parent. The gallery control is created
	 * using the given SWT style bits. The viewer has no input, no content provider, a default label provider, no sorter,
	 * and no filters.
	 * 
	 * @param parent
	 *          the parent control
	...
	/**
	 * Creates a gallery viewer on the given gallery control. The viewer has no input, no content provider, a default
	...
	/**
	 * <p>
	 * Gallery expects contents to have exactly 2 levels of hierarchy, with groups as the root elements and image
	 * thumbnails as direct children of the groups. This method accepts ITreeContentProvider and ITreePathContentProvider
	 * as-is, and relies on the providers to return contents with the correct structure.
	 * </p>
	 * <p>
	 * This method also accepts IStructuredContentProvider and wraps it in a FlatTreeContentProvider with an empty string
	 * as the root node. If you need a different root node, construct your own FlatTreeContentProvider and pass it here.
	 ...
	 *          the parent element, the input element, or a tree path to the parent element
	 * @param index
	 *          child index
	 * @since 3.3
	 */
	public void remove(final Object parentOrTreePath, final int index) {
		// if (isBusy())
		// return;
		final List<TreePath> oldSelection = new LinkedList<TreePath>(Arrays.asList(((TreeSelection) getSelection())
				.getPaths()));
		preservingSelection(new Runnable() {
			public void run() {
				...
				if (removedPath != null) {
					...
					TreePath toRemove = null;
					for (TreePath path : oldSelection) {
						if (path.startsWith(removedPath, getComparer())) {
							toRemove = path;
							removed = true;
						}
					}
					oldSelection.remove(toRemove);
					if (removed) {
						setSelection(new TreeSelection((TreePath[]) oldSelection.toArray(new TreePath[oldSelection.size()]),
								getComparer()), false);
					}
