...
public class CSashForm extends SashForm {
	...
	/**
	 * Custom style bits. They set whether hide to one side of the other is not permitted. For example, if NO_HIDE_UP,
	 * then there will be only one arrow. When not hidden, it will point down (and will do a hide down), and when hidden
	 * down, it will point up (and will do a restore to the previous weight). There won't be a hide to the top arrow.
	 */
	public static final int NO_HIDE_LEFT = 0x1, // Custom style bit for not allow hide left
	...
	private class SashInfo {
		...
		public int[][] sashLocs; // There is one entry for each arrow, It is arrowType/arrowDrawn/x/y/height/width of the
															// arrow area.
		// There may not be a second entry, in which case we have only one arrow.
		public Point[] savedSizes = new Point[2]; // Saved sizes of controls - saved whenever a control is hidden or
																							// restored
	...
	protected static final int UP_RESTORE_ARROW = 0, UP_HIDE_ARROW = 1, DOWN_RESTORE_ARROW = 2, DOWN_HIDE_ARROW = 3,

	HIDE_ARROWS = 4;

	protected static final int ARROW_TYPE_INDEX = 0, ARROW_DRAWN_INDEX = 1, X_INDEX = 2, Y_INDEX = 3, WIDTH_INDEX = 4,
			HEIGHT_INDEX = 5;
	...
	protected void recomputeSashInfo() {
		...
		int[] weights = getWeights(); // This should be two entries only. We shouldn't have got here if there were more than
																	// two.
		...
		if (weights[1] == 0) {
			...
			if (currentSashInfo.restoreWeight != NO_WEIGHT && sashBounds.y <= DRAG_MINIMUM) {
				slammed = SLAMMED_TO_TOP;
			} else if (currentSashInfo.restoreWeight != NO_WEIGHT
			...
			if (currentSashInfo.restoreWeight != NO_WEIGHT && sashBounds.x <= DRAG_MINIMUM) {
				slammed = SLAMMED_TO_TOP;
					...
					&& sashBounds.x + sashBounds.width >= clientArea.width - DRAG_MINIMUM) {
				slammed = SLAMMED_TO_BOTTOM;
			}
		}
		...
		if (noHideUp) {
			if (slammed == SLAMMED_TO_BOTTOM) {
				...
				currentSashInfo.restoreWeight = NO_WEIGHT; // Since we are in the middle, there is no restoreWeight. We've could
																										// of been dragged here.
				currentSashInfo.sashBorderLeft = sashBorders != null ? sashBorders[0] : false;
		...
		currentSashInfo.sash.redraw(); // Need to schedule a redraw because it has already drawn the old ones during the set
																		// bounds in super layout.
	}
	...
	/*
	 * This determines if the control or one of its children has the focus. Control.isFocusAncestor is hidden by SWT, but
	 * it is really useful.
	 */
	...
	protected void drawArrow(GC gc, int[] sashLoc, boolean selected) {
		...
		if (selected) {
			if (!inMouseClick) {
				...
				gc.drawLine(sashLoc[X_INDEX] + sashLoc[WIDTH_INDEX], sashLoc[Y_INDEX] + sashLoc[HEIGHT_INDEX], sashLoc[X_INDEX]
						+ sashLoc[WIDTH_INDEX], sashLoc[Y_INDEX]);
			} else {
