...
/**
 * Controls to change the font name of the selected items. Since this is a
 * complex toolitem (because the created toolitem contains a composite with
 * other controls) this need to be put in a toolbar where at least a static
 * simple item is present. In the context of the textual toolbar this is done
 * using the {@link DecrementFontSizeButtonContributionItem} and the
 * {@link IncrementFontSizeButtonContributionItem}
 * 
...
public class TextualFontNameContributionItem extends CommonToolbarHandler {
	...
	/**
	 * Listener used to check if the font contribution in the preferences are
	 * changed, and trigger the update of the combo
	 */
	private final class PreferenceListener implements IPropertyChangeListener {

		public void propertyChange(org.eclipse.jface.util.PropertyChangeEvent event) {
			if (event.getProperty().equals(FontsPreferencePage.FPP_FONT_LIST)) {
				// If the property change in the preferences force the refresh
				// of the fonts
				setAvailableFonts();
				setAllControlsData();
			}
	...
	/**
	 * The last font list set on the combo. Set or read the font list directly
	 * from the combo has a little overhead because the status of the widget is
	 * check (about 100ms in some cases) so it is better to compare it with this
	 * one first
	 */
	 ...
	 * @return Return true if the new font are different from the fonts
	 * previously stored false otherwise
	 */
	...
	public boolean isVisible() {
		...
		if (selectionValid) {
			...
			IResource r = (IResource) n.getJasperConfiguration().get(FileUtils.KEY_FILE);
	...
	/**
	 * Initialize all the controls that show the state of the object with the
	 * value of the selected element
	 */
	protected void setAllControlsData() {
		if (fontName == null || fontName.isDisposed())
			return;
		refreshing = true;
		List<Object> selection = getSelectionForType(MTextElement.class);
		if (selection.size() == 1) {
	 ...
	 * has default value inside the node. Also it will contain the action to set
	 * the value to null if the operation is allowed.
	 * 
	 * Since on mac the combo item doens't have a contextual menu it add a
	 * special listneer for them as workaround to the problem
	 * 
	 * @param node node where the the command will be executed and from where
	 * the default map is extracted
	...
	protected void createContextualMenu(final APropertyNode node, final Control control, final String propertyID) {
		if (node != null && control != null && !control.isDisposed()) {
			...
			if (!entryCreated) {
				// if no entry was created remove the contextual menu, but first
				// dispose
				// the old one
				if (control.getMenu() != null) {
					control.getMenu().dispose();
