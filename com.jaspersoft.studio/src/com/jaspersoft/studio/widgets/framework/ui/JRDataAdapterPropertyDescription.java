...
/**
 * Widget descriptor used to select the path of a JasperReports data adapter. It
 * provide a combo to select a local data adapter and a button to select a data
 * adapter from a path. It also allow direct typing of the path
 * 
...
public class JRDataAdapterPropertyDescription extends AbstractExpressionPropertyDescription<String> {
	...
	public JRDataAdapterPropertyDescription(String name, String label, String description, boolean mandatory,
	...
	public Control createControl(final IWItemProperty wiProp, Composite parent) {
		...
		// create the combo
		final WritableComboMenuViewer viewer = new WritableComboMenuViewer(simpleContainer,
				WritableComboMenuViewer.NO_IMAGE);
			...
			for (int i = 0; i < daStorage.length; i++) {
				...
				if (!(s instanceof PreferencesDataAdapterStorage)) {
					for (DataAdapterDescriptor d : s.getDataAdapterDescriptors(currentDataset)) {
						Pair<DataAdapterDescriptor, ADataAdapterStorage> value = new Pair<>(d, s);
		...
		viewer.addSelectionListener(new ComboItemAction() {
			...
			public void exec() {
				...
				if (selectedElement.getValue() instanceof Pair) {
					...
					Pair<DataAdapterDescriptor, ADataAdapterStorage> value = (Pair<DataAdapterDescriptor, ADataAdapterStorage>) selectedElement
							.getValue();
	...
	public ItemPropertyDescription<?> getInstance(WidgetsDescriptor cd, WidgetPropertyDescriptor cpd,
			JasperReportsConfiguration jConfig) {
		JRDataAdapterPropertyDescription result = new JRDataAdapterPropertyDescription(cpd.getName(),
