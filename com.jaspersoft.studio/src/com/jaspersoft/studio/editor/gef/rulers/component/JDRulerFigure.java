...
public class JDRulerFigure extends Figure {
	...
	protected void paintFigure(Graphics graphics) {
		/*
		 * @TODO:Pratik maybe you can break this method into a few methods. that might make it a little easier to read and
		 * understand. plus, sub-classes could customize certain parts.
		 */
		...
		/*
		 * A major mark is one that goes all the way from the left edge to the right edge of a ruler and for which a number
		 * is displayed. Determine the minimum number of pixels that are to be left between major marks. This will, in turn,
		 * help determine how many units are to be displayed per major mark. A major mark should have at least enough pixels
		 * to display the text and its padding. We take into the consideration the max of text's width and height so that
		...
		if (interval > 0) {
			/*
			 * If the client specified how many units are to be displayed per major mark, use that. If, however, showing that
			 * many units wouldn't leave enough room for the text, than take its smallest multiple that would leave enough
			 * room.
			 */
		...
		/*
		 * divsPerMajorMark indicates the number of divisions that a major mark should be divided into. for eg., a value of
		 * 2 would mean that a major mark would be shown as having two parts. that means that there would be a marker
		 * showing the beginning and end of the major marker and another right in the middle.
		 */
		int divsPerMajorMark;
		if (divisions > 0 && dotsPerUnit * unitsPerMajorMark / divisions >= minPixelsBetweenMarks) {
			/*
			 * If the client has specified the number of divisions per major mark, use that unless it would cause the minimum
			 * space between marks to be less than minPixelsBetweenMarks
			 */
			divsPerMajorMark = divisions;
		} else {
			/*
			 * If the client hasn't specified the number of divisions per major mark or the one that the client has specified
			 * is invalid, then calculate it dynamically. This algorithm will try to display 10 divisions per CM, and 16 per
			 * INCH. However, if that puts the marks too close together (i.e., the space between them is less than
			 * minPixelsBetweenMarks), then it keeps decreasing the number of divisions by a factor of 2 until there is enough
			 * space between them.
			 */
			divsPerMajorMark = 2;
			if (getUnit() == RulerProvider.UNIT_CENTIMETERS) {
		...
		/*
		 * mediumMarkerDivNum is used to determine which mark (line drawn to indicate a point on the ruler) in a major mark
		 * will be of medium size. If its value is 1 then every mark will be of medium size. If its value is 5, then every
		 * 5th mark will be of medium size (the rest being of small size).
		 */
		...
		/*
		 * dotsPerDivision = number of pixels between each mark = number of pixels in a division
		 */
		double dotsPerDivision = dotsPerUnit * unitsPerMajorMark / divsPerMajorMark;
		/*
		 * startMark is the division/mark from which we are going to start painting. It should be the last major mark (one
		...
		for (int div = startMark; div <= endMark; div++) {
			...
			int y = (int) ((int) (div * dotsPerDivision) + ((isHorizontal() ? hoffset : voffset) * zoomManager.getZoom()));
			if (div % divsPerMajorMark == 0) {
				...
				if (isHorizontal()) {
					...
					/*
					 * If the width is even, we want to increase it by 1. This will ensure that when marks are erased because they
					 * are too close to the number, they are erased from both sides of that number.
					 */
					if (numSize.width % 2 == 0)
						numSize.width++;
					Point textLocation = new Point(y - (numSize.width / 2), clippedBounds.x + textMargin - leading - 1);
		...
		drawMargins(graphics, clippedBounds);
		// paint the border
	...
	/**
	 * Allows the client to set the number of units to be displayed per major mark, and the number of divisions to be
	 * shown per major mark.
	 * 
	 * A number on the ruler is considered to be a major mark.
	 * 
	 * @param unitsPerMajorMark
	 *          if less than 1, it will be ignored; if there is not enough space to display that many units per major
	 *          mark, its smallest multiple that leaves enough room will be used.
	 * @param divisionsPerMajorMark
	 *          if less than 1, it will be ignored; if displaying that many divisions does not leave enough room between
	 *          marks, it will be ignored.
	 * 
	 */
