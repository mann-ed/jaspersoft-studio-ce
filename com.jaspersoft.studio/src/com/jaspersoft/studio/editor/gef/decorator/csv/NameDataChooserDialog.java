 ...
 * 
 */
public class NameDataChooserDialog extends ATitledDialog {
	 ...
	 * @param mtext
	 */
	public NameDataChooserDialog(Shell parentShell, MTextElement mtext) {
		super(parentShell);
		this.mtext = mtext;
		setTitle(Messages.NameDataChooserDialog_0);
		setDescription(Messages.NameDataChooserDialog_1);
		JRDesignElement v = mtext.getValue();
		path = v.getPropertiesMap().getProperty(JRCsvMetadataExporter.PROPERTY_COLUMN_NAME);
		if (path == null) {
		...
		repeat = Misc.nvl(Boolean.valueOf(v.getPropertiesMap().getProperty(JRCsvMetadataExporter.PROPERTY_REPEAT_VALUE)),
				false);
	...
	protected Control createDialogArea(Composite parent) {
		...
		new Label(cmp, SWT.NONE).setText(Messages.NameDataChooserDialog_2);
	...
	private void validate(String path) {
		...
		if (Misc.isNullOrEmpty(path)) {
			...
			if (map != null && map.getProperty(JRCsvMetadataExporter.PROPERTY_COLUMN_NAME) != null)
