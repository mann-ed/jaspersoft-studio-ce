...
public class Thumbnail extends Figure implements UpdateListener {
	...
	/**
	 * This updates the Thumbnail by breaking the thumbnail {@link Image} into several tiles and updating each tile
	 * individually.
	 */
	class ThumbnailUpdater implements Runnable {
		 ...
		 * different. The current tile indexes are incremented and if more updating is necesary, this {@link Runnable} is
		 * called again in a {@link Display#timerExec(int, Runnable)}. If no more updating is required, {@link #stop()} is
		 * called.
		 */
		public void run() {
		...
		/**
		 * Starts this updater. This method initializes all the necessary resources and puts this {@link Runnable} on the
		 * asynch queue. If this updater is not active or is already running, this method just returns.
		 */
		public void start() {
			...
			BufferedImage offsceeenBufferedImage = new BufferedImage(e.width, e.height, 2);
			Graphics2D graphics2d = offsceeenBufferedImage.createGraphics();

			thumbnailGC = new GC(thumbnailImage, sourceFigure.isMirrored() ? SWT.RIGHT_TO_LEFT : SWT.NONE);
			thumbnailGraphics = new J2DScaledGraphics(new SWTGraphics(thumbnailGC), thumbnailGC, graphics2d);
			thumbnailGraphics.scale(getScaleX());

			thumbnailGraphics.translate(e.getLocation().negate());
			...
			setScales(targetSize.width / (float) e.width, targetSize.height / (float) e.height);
		...
		/**
		 * Stops this updater. Also disposes of resources (except the thumbnail image which is still needed for painting).
		 */
		public void stop() {
			isRunning = false;
	...
	/**
	 * Returns the preferred size of this Thumbnail. The preferred size will be calculated in a way that maintains the
	 * source Figure's aspect ratio.
	 * 
	 * @param wHint
	 *          The width hint
	 * @param hHint
	...
	/**
	 * Sets the X and Y scales for the Thumbnail. These scales represent the ratio between the source figure and the
	 * Thumbnail.
	 * 
	 * @param x
	 *          The X scale
