...
public class ExcelExporterPreferencePage extends FieldEditorOverlayPage {
	...
	private void createSheet(CTabFolder tabFolder) {
		...
		JSSComboFieldEditor cfe = new JSSComboFieldEditor(NSF_EXPORT_XLS_SHEET_DIRECTION,
				Messages.ExcelExporterPreferencePage_39,
				new String[][] { { RunDirectionEnum.LTR.getName(), RunDirectionEnum.LTR.getName() },
						{ RunDirectionEnum.RTL.getName(), RunDirectionEnum.RTL.getName() } },
				parent);
		addField(cfe);
		HelpSystem.setHelp(cfe.getComboBoxControl(parent),
		...
		addField(iedit);
		HelpSystem.setHelp(iedit.getTextControl(parent),
				StudioPreferencePage.REFERENCE_PREFIX + iedit.getPreferenceName());
		...
		addField(bf);
		HelpSystem.setHelp(bf.getDescriptionControl(parent),
				StudioPreferencePage.REFERENCE_PREFIX + bf.getPreferenceName());
	...
	private void createTabCells(CTabFolder tabFolder) {
		...
		BooleanFieldEditor bf = new BooleanFieldEditor(NSF_EXPORT_XLS_CELL_HIDDEN,
				Messages.ExcelExporterPreferencePage_53, sc);
		addField(bf);
		HelpSystem.setHelp(bf.getDescriptionControl(sc),
				StudioPreferencePage.REFERENCE_PREFIX + bf.getPreferenceName());
	...
	public static void getDefaults(IPreferenceStore store) {
		...
		store.setDefault(XlsReportConfiguration.PROPERTY_IMAGE_BORDER_FIX_ENABLED, Misc.nvl(
		...
		// sheet
		store.setDefault(NSF_EXPORT_XLS_SHEET_DIRECTION, Misc.nvl(
				PropertiesHelper.DPROP.getProperty(NSF_EXPORT_XLS_SHEET_DIRECTION), RunDirectionEnum.LTR.getName()));
		...
		store.setDefault(NSF_EXPORT_XLS_FIT_HEIGHT,
				Misc.nvl(PropertiesHelper.DPROP.getProperty(NSF_EXPORT_XLS_FIT_HEIGHT))); // $NON-NLS-1$
		store.setDefault(NSF_EXPORT_XLS_FIT_WIDTH,
				Misc.nvl(PropertiesHelper.DPROP.getProperty(NSF_EXPORT_XLS_FIT_WIDTH))); // $NON-NLS-1$
		// CELL
		...
		store.setDefault(NSF_EXPORT_XLS_CELL_WRAP_TEXT,
				PropertiesHelper.DPROP.getProperty(NSF_EXPORT_XLS_CELL_WRAP_TEXT));

		store.setDefault(NSF_EXPORT_XLS_AUTO_FILTER,
				Misc.nvl(PropertiesHelper.DPROP.getProperty(NSF_EXPORT_XLS_AUTO_FILTER)));
		store.setDefault(NSF_EXPORT_XLS_FREEZ_ROW,
				Misc.nvl(PropertiesHelper.DPROP.getProperty(NSF_EXPORT_XLS_FREEZ_ROW)));
		store.setDefault(NSF_EXPORT_XLS_FREEZ_COLUMN,
				Misc.nvl(PropertiesHelper.DPROP.getProperty(NSF_EXPORT_XLS_FREEZ_COLUMN)));

		store.setDefault(XlsReportConfiguration.PROPERTY_FONT_SIZE_FIX_ENABLED, Misc.nvl(
				PropertiesHelper.DPROP.getProperty(XlsReportConfiguration.PROPERTY_FONT_SIZE_FIX_ENABLED), "false")); //$NON-NLS-1$
	}
