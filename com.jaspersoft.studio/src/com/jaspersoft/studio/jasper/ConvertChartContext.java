 ...
 * @version $Id: ConvertChartContext.java 5878 2013-01-07 20:23:13Z teodord $
 */
public class ConvertChartContext implements ChartContext {
	private final JRChart chart;
	private JasperReportsContext context;

	protected ConvertChartContext(JRChart chart, JasperReportsContext context) {
		this.chart = chart;
	...
	private IntervalCategoryDataset createGanttDataset() {
		if (sampleGanttDataset == null) {
			...
			series
					.add(new Task("Second", new SimpleTimePeriod(date(9, Calendar.APRIL, 2008), date(9, Calendar.APRIL, 2008))));
