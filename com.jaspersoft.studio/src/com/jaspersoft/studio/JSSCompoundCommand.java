...
/**
 * Special compound command that disable the refresh of the editor and of the
 * selection during the execution of the commands, this is done for performance
 * improvements and avoid unnecessary selections
 * 
...
public class JSSCompoundCommand extends CompoundCommand {
	...
	/**
	 * Node used to go back into the model until the mpage or an mreport ( or
	 * more generally an MLockableRefresh) is found. In this class, for every
	...
	/**
	 * Command used to set the selection ad the end of the inner commands
	 * execution. This is used only when the restoreSelection flag is set to
	 * true
	 */
	private SelectElementCommand restoreSelectionCommand = null;

	/**
	 * Create an instance of the from a compound command. All the commands
	 * inside the compound command, and eventually its name, will be also
	 * present in the new JSSCompoundCommand
	 * 
	 * @param cmd a not null compound command
	 * @param referenceNode a node of the model manipulated by the inner
	 * commands. Can be null, in this case this command works like a standard
	 * compound command
	 */
	...
	/**
	 * Set the reference node node if and only if the actual value is null. the
	 * passed node is used to get the root is possible and that is used as
	 * reference node. This is done because maybe the node will be deleted by
	 * another command in the meantime, so during the execution it could be
	 * outside the hierarchy. The root instead is never deleted
	 * 
	...
	/**
	 * Set the reference node node if and only if the actual value is null and
	 * the value is an ANode. the passed node is used to get the root is
	 * possible and that is used as reference node. This is done because maybe
	 * the node will be deleted by another command in the meantime, so during
	 * the execution it could be outside the hierarchy. The root instead is
	 * never deleted
	 * 
	 * @param referenceNode the reference node
	 */
	 ...
	 * If the node dosen't bring to the root of the model then it fallback using
	 * the model inside the opened editor
	 * 
	...
	/**
	 * Take a node and go back into it until it find a MLockableRefresh node.
	 * During the search the reference node it is updated so when it will be
	 * found it will be substitute the actual reference node, to speedup the
	 * next research
	 * 
	 * @return a MLockableNode or null if it can not be found
	 */
	protected ANode getNode() {
		if (referenceNode != null) {
			if (referenceNode instanceof MLockableRefresh)
				return referenceNode;
	...
	protected void refreshVisuals() {
		try {
			...
			if (report != null) {
				PropertyChangeEvent event = new PropertyChangeEvent(report.getJasperDesign(), REFRESH_UI_EVENT, null,
						null);
	...
	/**
	 * Create a fake command to force the refresh of the editor and outline
	 * panel on the passed node, this override the disable refresh flag, so
	 * calling this the editor area is always updated
	 */
	...
	/**
	 * Set to ignore or not the refresh on the root of the model of the actual
	 * reference node
	 * 
	...
	protected void setIgnoreEvents(boolean value) {
		...
		if (rootNode != null) {
			if (rootNode instanceof MLockableRefresh)
				((MLockableRefresh) rootNode).setIgnoreEvents(value, this);
		}
	}
	...
	/**
	 * If the restoreSelection flag is enable this call will backup the
	 * selection of the current editor an create the command to restore it
	 */
			...
			if (editor != null) {
				...
				if (designEditor instanceof AbstractVisualEditor) {
					...
					if (viewer != null) {
						...
						ArrayList<JRChild> oldSelection = new ArrayList<>();
						for (Object obj : selectedParts) {
							...
							if (part.getModel() instanceof ANode) {
								...
								if (model.getValue() instanceof JRDesignElement
										|| model.getValue() instanceof JRDesignBand) {
	...
	/**
	 * If the command to restore the seleciton is available then it is executed
	 */
	...
	/**
	 * Override of the execute command, disable the refresh before the first
	 * command and enable it at the end
	 */
	@Override
	public void execute() {
		if (size() > 0) {
			...
			// Since there could be multiple compound command executed at the
			// same time, the refresh
			// is not forced if the actual command is not the last lock removed.
			// In other words the refresh
	...
	/**
	 * Override of the undo command, disable the refresh before the first
	 * command and enable it at the end
	 */
	@Override
	public void undo() {
	...
	/**
	 * Set the selection restore flag. The selection restore works by storing
	 * the selection of the current editor before the execution of the inner
	 * commands and restoring it after the commands are executed. By default
	 * this is not done. Also the backup and the selection of the elements relay
	 * on their JRValue and not on the nodes
	 * 
	 * @param enabled true if the selection backup and restore should be
