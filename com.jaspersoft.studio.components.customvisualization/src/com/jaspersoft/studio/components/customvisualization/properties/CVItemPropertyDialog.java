...
public class CVItemPropertyDialog extends PersistentLocationDialog implements IExpressionContextSetter {
	...
	public CVItemPropertyDialog(Shell parentShell, ItemProperty itemProperty,
	...
	protected Control createDialogArea(Composite parent) {
		...
		lblPropertyName.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				false, 1, 1));
		...
		if (propertiesFileLocation != null) {
			...
			infoPanel.SetDoubleClickListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					propertyName.setText(infoPanel.getSelectedElement()
							.getName());
				}
			});
		}
	...
	private List<ElementDescription> getPropertiesInformation() {
		...
		try {
			descriptions.addAll(ElementDescription
					.getPropertiesInformation(CustomVisualizationActivator
							.getDefault().getFileLocation(
	...
	private void initWidgets() {
		...
		if (this.itemProperty.getValue() != null) {
			...
			propertyValueExpression
					.setExpression((JRDesignExpression) itemProperty
							.getValueExpression());
	...
	private void addListeners() {
		...
		propertyValueExpression
				.addModifyListener(new ExpressionModifiedListener() {
					@Override
					public void expressionModified(ExpressionModifiedEvent event) {
						itemProperty
								.setValueExpression(event.modifiedExpression);
					}
				});
