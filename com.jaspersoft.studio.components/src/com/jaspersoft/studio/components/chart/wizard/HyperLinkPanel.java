...
/**
 * 
 * This class can create a composite with inside the widgets to edit an
 * MHyperLink node
 * 
...
public class HyperLinkPanel {
	...
	/**
	 * Element hider for the elements related to the anchor, so the label an the
	 * expression field\button
	 */
	private ElementHider anchor;

	/**
	 * Element hider for the elements related to the page, so the label an the
	 * expression field\button
	 */
	private ElementHider page;

	/**
	 * Element hider for the elements related to the reference, so the label an
	 * the expression field\button
	 */
	private ElementHider reference;

	/**
	 * Element hider for the elements related to the tooltip, so the label an
	 * the expression field\button
	 */
	private ElementHider tooltip;

	/**
	 * Element hider for the elements related to the parameters, so the label an
	 * the expression field\button
	 */
	private ElementHider parameters;

	/**
	 * Widget of the anchor field (only the field to insert the data, not the
	 * label with the name of the field)
	 */
	private WTextExpression anchorWidget;

	/**
	 * Widget of the reference field (only the field to insert the data, not the
	 * label with the name of the field)
	 */
	private WTextExpression referenceWidget;

	/**
	 * Widget of the parameters field (only the field to insert the data, not
	 * the label with the name of the field)
	 */
	private WTParametersText parametersWidget;

	/**
	 * Widget of the when field (only the field to insert the data, not the
	 * label with the name of the field)
	 */
	private WTextExpression whenWidget;

	/**
	 * Widget of the tooltip field (only the field to insert the data, not the
	 * label with the name of the field)
	 */
	private WTextExpression tooltipWidget;

	/**
	 * Widget of the page field (only the field to insert the data, not the
	 * label with the name of the field)
	 */
	private WTextExpression pageWidget;

	/**
	 * Control of the target selection field (only the field to insert the data,
	 * not the label with the name of the field)
	 */
	private Combo targetCombo;

	/**
	 * Control of the type selection field (only the field to insert the data,
	 * not the label with the name of the field)
	 */
	...
	/**
	 * Hashmap that contains, for every hyperlink type, a list of hiders. Every
	 * one of these hider contains one or more controls that should be visible
	 * with the type.
	 * 
	 */
	 ...
	 */
	private static String[] linkTargetItems = new String[] { HyperlinkTargetEnum.SELF.getName(),
			HyperlinkTargetEnum.BLANK.getName(), HyperlinkTargetEnum.TOP.getName(),
	...
	static {
		...
		filteredTypes.add(HyperlinkTypeEnum.CUSTOM); // Will be used
														// automatically when
														// user write a custom
														// entry
		filteredTypes.add(HyperlinkTypeEnum.NULL); // Makes no much sense into
													// this widget
	...
	/**
	 * This class can contains an array of controls and provide the methods to
	 * easily show or hide that controls. The controls must be inside a grid
	 * layout
	 * 
	 * @author Orlandin Marco
	 *
	...
	private class ElementHider {
		 ...
		 * 
		 * @param controls
		 *            the controls to show or hide, they must be into a grid
		 *            layout. Since the grid data is needed, if the control has
		 *            not a grid data then a default one will be assigned
		 */
	 ...
	 * 
	 * @return a grid data with an horizontal span of two, and an horizontal
	 *         filling. At every call it is returned a new instance of the grid
	 *         data
	 */
	...
	/**
	 * Refresh the contents of the controls, setting into them the data of the
	 * selected element
	 */
	public void refresh() {
		APropertyNode element = getElement();
		if (element != null) {
			setExpressionComponentData(anchorWidget, element,
					element.getPropertyActualValue(JRDesignHyperlink.PROPERTY_HYPERLINK_ANCHOR_EXPRESSION));
			setExpressionComponentData(referenceWidget, element,
					element.getPropertyActualValue(JRDesignHyperlink.PROPERTY_HYPERLINK_REFERENCE_EXPRESSION));
			setExpressionComponentData(whenWidget, element,
					element.getPropertyActualValue(JRDesignHyperlink.PROPERTY_HYPERLINK_WHEN_EXPRESSION));
			setExpressionComponentData(pageWidget, element,
					element.getPropertyActualValue(JRDesignHyperlink.PROPERTY_HYPERLINK_PAGE_EXPRESSION));
			setExpressionComponentData(tooltipWidget, element,
					element.getPropertyActualValue(JRDesignHyperlink.PROPERTY_HYPERLINK_TOOLTIP_EXPRESSION));
			...
			// I don't set the text on the combo if it has already the right
			// value to avoid to raise the panel refresh
			if (!typeValue.equals(typeCombo.getText()))
	...
	private void createMap() {
		if (hideList == null) {
			...
			hideList.put(linkTypeItems[1], new ElementHider[] { tooltip, reference });
			hideList.put(linkTypeItems[2], new ElementHider[] { tooltip, anchor });
			hideList.put(linkTypeItems[3], new ElementHider[] { tooltip, page });
			hideList.put(linkTypeItems[4], new ElementHider[] { tooltip, reference, anchor });
			hideList.put(linkTypeItems[5], new ElementHider[] { tooltip, reference, page });
			hideList.put(linkTypeItems[6], new ElementHider[] { tooltip, parameters });
			hideList.put("Custom", new ElementHider[] { tooltip, parameters, reference, anchor, page });
		}
	}

	/**
	 * Show or hide the visible controls using the the link type actual
	 * selection to choose which show
	 */
	private void refreshVisibleComponents() {
		...
		if (!hideList.containsKey(selectedValue))
	...
	/**
	 * Read the textual value of a combo and set it into a property inside the
	 * element
	 * 
	 * @param combo
	 *            combo from where the value is read
	 * @param property
	...
	private void readValueFromCombo(Combo combo, String property) {
		...
		if (element != null) {
			...
			// Since it is called even on the combo modify it will move the
			// cursor on the start at every call
			// causing that every character typed reset the cursor on the
			// beginning. This will put the cursor on the end
			combo.setSelection(new Point(stringLength, stringLength));
		}
	}

	/**
	 * Create a label and set its content text and its tooltip text, then return
	 * it
	 * 
	 * @param parent
	...
	/**
	 * Create a WTextExpression component and set the graphical appearance. This
	 * component can be used to easily input an expression
	 * 
	 * @param parent
	 ...
	 *            the component
	 * @param pnode
	 *            Node from where the expression context will be extracted,
	 *            should be also the node that will contains the expression
	 * @param b
	 *            the JRDesignExpression expression to set inside the
	 *            WTextExpression component
	 */
	...
	/**
	 * Set an expression property of the element with the expression extracted
	 * from a WTextExpression widget
	 * 
	 * @param propertyId
	 *            the id of the expression property inside the element
	 * @param expressionWidget
	 *            the widget from where the expression value is extracted
	 */
	...
	/**
	 * Update the values inside the MHyperlink property node with the value
	 * defined inside the expression widgets. The parameters are not updated
	 * here since the are update on the fly when the appropriate widget is used
	 */
	 ...
	 * 
	 * @param parent
	 *            parent where all the controls will be created
	 */
	public void createControls(Composite parent) {
		...
		Label anchorLabel = createLabel(mainComposite, Messages.MHyperLink_hyperlink_anchor_expression_description,
				Messages.MHyperLink_hyperlink_anchor_expression);
		...
		Label pageLabel = createLabel(mainComposite, Messages.MHyperLink_hyperlink_page_expression_description,
				Messages.MHyperLink_hyperlink_page_expression);
		...
		Label referenceLabel = createLabel(mainComposite,
				Messages.MHyperLink_hyperlink_reference_expression_description,
				Messages.MHyperLink_hyperlink_reference_expression);
		...
		Label tooltipLabel = createLabel(mainComposite, Messages.MHyperLink_hyperlink_tooltip_expression_description,
				Messages.MHyperLink_hyperlink_tooltip_expression);
		...
		Label parametersLabel = createLabel(mainComposite, Messages.MHyperLink_parameters_description,
				Messages.common_parameters);
		parametersWidget = new WTParametersText(mainComposite, JRDesignHyperlink.PROPERTY_HYPERLINK_PARAMETERS,
				getElement());
