...
/**
 * Template engine to build a report with a table in the summary, from a
 * TableTemplate
 * 
...
public class TableTemplateEngine extends DefaultTemplateEngine {
	...
	/**
	 * The list of styles that will applied to the table, the styles order is
	 * important, and it should be Table Style, Table Header, Column Header and
	 * detail
	 */
	...
	/**
	 * Create a column of the table of the specified width. The section in the new
	 * column will be the same one of the column of the template and also the
	 * content. The size and position of the content will be relative to the width
	 * of the new column that may be different from the one of the template.
	 * 
	 * 
	 * @param tbl
	 *            the table
	 ...
	 *            the column width
	 */
	private StandardColumn generateColumn(StandardTable tbl, JRPropertiesMap tableMap, JasperDesign jd,
			String fieldName, String fieldValue, int colWidth) {
		StandardColumn col = CreateColumnCommand.addColumnWithStyle(jd, tbl, tableMap, sections.isTableHeader(),
				sections.isTableFooter(), sections.isColumnHeader(), sections.isColumnFooter(),
	...
	/**
	 * Create a group column of the table, with a width of the whole table. the
	 * content in the new column The size and position of the content will be
	 * relative to the width of the new column that may be different from the one of
	 * the template.
	 * 
	 * 
	 * @param groupCell
	 *            cell of the group where the element read from the template are
	 *            placed
	 * @param groupIndex
	 *            number of the group
	 * @param newGroupWidth
	 *            width of the group in the resulting table
	 * @param groupExpression
	 *            the expression of the groupfield
	 * @param jd
	 *            the jasperdesign
	 */
	private void createGroupCell(DesignCell groupCell, int groupIndex, int newGroupWidth,
			JRDesignExpression groupExpression, JasperDesign jd) {
	...
	/**
	 * Check if a JRDesignText element is a placeholder for the static text
	 * appearance in the column header
	 * 
	 * @param element
	 *            the element
	...
	/**
	 * Check if a JRDesignTextField element is a placeholder for the text field
	 * appearance in the detail
	 * 
	 * @param element
	 *            the element
	 * @return true if it is a place holder, false otherwise
	...
	/**
	 * Calculate a relative width for an element using a proportion between the
	 * width of the column in the template and the width in the generated report
	 * 
	 * @param newColwidth
	 *            width of the column in the report
	 * @param elementWidth
	 *            width of the element in the template
	 ...
	 *            the column width
	 * @param parentCol
	 *            the group where the column will be add
	 */
	protected void createGroupColumn(StandardTable tbl, JRPropertiesMap tableMap, JasperDesign jd, String fieldName,
			String fieldValue, int colWidth, StandardColumnGroup parentCol) {
	 ...
	 *            the column width
	 */
	private void createColumn(StandardTable tbl, JRPropertiesMap tableMap, JasperDesign jd, String fieldName,
			String fieldValue, int colWidth) {
	 ...
	 * 
	 * @param col
	 *            a column, used to calculate the height
	...
	private int getTableHeight(BaseColumn col) {
		...
		if (col.getTableHeader() != null)
			height += col.getTableHeader().getHeight();
		if (col.getTableFooter() != null)
			height += col.getTableFooter().getHeight();
		if (col.getColumnHeader() != null)
			height += col.getColumnHeader().getHeight();
		if (col.getColumnFooter() != null)
			height += col.getColumnFooter().getHeight();
		...
		if (col instanceof StandardColumn) {
			...
			if (standardCol.getDetailCell() != null)
				height += standardCol.getDetailCell().getHeight();
		}
		return height;
	}
	 ...
	 * 
	 * @param jd
	 *            The JasperDesign of the report where the table will be placed
	 * @param datasetRun
	...
	private JRDesignElement getTable(JasperDesign jd, JRDesignDatasetRun datasetRun) {
		...
		if (tableFields != null && tableFields.size() > 0) {
			...
			if (columnWidth * tableFields.size() > tableWidth)
				colWidth = tableWidth / tableFields.size();
			if (sections == null)
				sections = TableWizardLayoutPage.getDefaultSection();
			...
			if (groupFields != null && groupFields.size() > 0) {
				...
				for (Object f : tableFields) {
					createGroupColumn(tbl, tableMap, jd, ((JRField) f).getName(), "$F{" + ((JRField) f).getName() + "}", //$NON-NLS-1$ //$NON-NLS-2$
							colWidth, parentCol);
				}
				int height = sections.getGroupHeaderHeight();
				// Create a spanned cell inside the column group, that take the field with the
				// name of the group
				int groupIndex = 0;
				for (Object field : groupFields) {
					...
					JRDesignExpression groupExpression = ExprUtil.setValues(new JRDesignExpression(),
							"$F{" + groupField.getName() + "}", groupField.getValueClassName()); //$NON-NLS-1$ //$NON-NLS-2$
				...
				if (tableHeight < minimumHeight)
					tableHeight = minimumHeight;
			} else {
				// There are no groups, so will not be created group columns
				for (Object f : tableFields) {
					createColumn(tbl, tableMap, jd, ((JRField) f).getName(), "$F{" + ((JRField) f).getName() + "}", //$NON-NLS-1$ //$NON-NLS-2$
							colWidth);
				}
		...
		// Create and apply the styles to the table. The styles should be read from the
		// template report
		// if for some reason this styles are not present then a default set of styles
		// will be used
		ApplyTableStyleAction applyAction;
		if (stylesList != null)
			applyAction = new ApplyTableStyleAction(stylesList, jrElement);
		else
			applyAction = new ApplyTableStyleAction(TableWizardLayoutPage.getDefaultStyle(), jrElement);
	...
	/**
	 * From an array of JRDesignStyle try to find the style to apply to the table.
	 * The styles searched must have a specific name: Table, Table_TH, Table_CH,
	 * Table_TD. If all this four styles are found then a List with their references
	 * is returned, otherwise some default styles will be generated and returned
	 * 
	 * @param styleArray
	 *            the array of JRDesignStyle
	...
	private List<JRDesignStyle> buildStylesList(JasperReportsContext jrContext, JasperDesign jd) {
		...
		if (result[1] == null || result[2] == null || result[3] == null) {
			...
			TableStyle defaultPattern = new TableStyle(AlfaRGB.getFullyOpaque(ColorConstants.white.getRGB()),
					ColorSchemaGenerator.SCHEMAS.PALE, BorderStyleEnum.FULL,
					AlfaRGB.getFullyOpaque(ColorConstants.black.getRGB()), false);
	...
	/**
	 * Search the template table inside a JasperDesign, looking in the children of
	 * the summary band
	 * 
	 * @param jd
	 *            the jasperdesign that contains the sample table
	 * @return a JRDesignComponentElement that contains a StandardTable, or null if
	 *         it isn't found
	 */
	...
	protected void processTemplate(JasperReportsContext jrContext, JasperDesign jd, List<Object> fields,
		...
		/**
		 * If the template table is found it will be used to create the style of the
		 * real table and of its content
		 */
		if (tableComponent != null) {
			...
			if (table.getColumns().size() > 0) {
				...
				sections = new TableSections(tableHeader, tableFooter, columnHeader, columnFooter, groupHeader,
						groupFooter);
				if (tableHeader)
					sections.setTableHeaderHeight(col.getTableHeader().getHeight());
				if (tableFooter)
					sections.setTableFooterHeight(col.getTableFooter().getHeight());
				if (columnHeader)
					sections.setColumnHeaderHeight(col.getColumnHeader().getHeight());
				if (columnFooter)
					sections.setColumnFooterHeight(col.getColumnFooter().getHeight());
				if (groupHeader)
					sections.setGroupHeaderHeight(col.getGroupHeaders().get(0).getCell().getHeight());
				if (groupFooter)
					sections.setGroupHeaderHeight(col.getGroupFooters().get(0).getCell().getHeight());
			...
			JRDesignStaticText colHeaderLabel = DefaultTemplateEngine.findStaticTextElement(jd.getColumnHeader(),
					"Label"); //$NON-NLS-1$
			JRDesignTextField cellField = DefaultTemplateEngine
					.findTextFieldElement(jd.getDetailSection().getBands()[0], "Field"); //$NON-NLS-1$
			tableHeaderContent = new ArrayList<JRDesignElement>();
			tableFooterContent = new ArrayList<JRDesignElement>();
			colFooterContent = new ArrayList<JRDesignElement>();
			colHeaderContent = new ArrayList<JRDesignElement>();
			...
			if (colHeaderLabel != null)
				colHeaderContent.add(colHeaderLabel);
			if (cellField != null)
				detailContent.add(cellField);
	...
	public static JRDesignStaticText findStaticTextElement(StandardTable parent, String exp) {
		...
		if (col != null) {
			...
			if (col.getTableHeader() != null)
				result = DefaultTemplateEngine.findStaticTextElement(col.getTableHeader(), exp);
			if (col.getColumnHeader() != null && result == null)
				result = DefaultTemplateEngine.findStaticTextElement(col.getColumnHeader(), exp);
			if (col.getDetailCell() != null && result == null)
				result = DefaultTemplateEngine.findStaticTextElement(col.getDetailCell(), exp);
			return result;
		}
		return null;
	}
	...
	public static JRDesignTextField findTextFieldElement(StandardTable parent, String exp) {
		...
		if (col != null) {
			...
			if (col.getTableHeader() != null)
				result = DefaultTemplateEngine.findTextFieldElement(col.getTableHeader(), exp);
			if (col.getColumnHeader() != null && result == null)
				result = DefaultTemplateEngine.findTextFieldElement(col.getColumnHeader(), exp);
			if (col.getDetailCell() != null && result == null)
				result = DefaultTemplateEngine.findTextFieldElement(col.getDetailCell(), exp);
			return result;
		}
		return null;
	}
	...
	/**
	 * Return the first standard group column, drilling down if the current column
	 * is a group
	 * 
	 ...
	 * @param the
	 *            column to check
	 * @return the parameter column casted to the correct type if it is a standard
	 *         group column null otherwise
	 */
	...
	public void setReportDataAdapter(ReportBundle bundle, DataAdapterDescriptor dataadapter, JRPropertiesMap properties,
		...
		for (String key : properties.getPropertyNames()) {
			...
			if (key.contains("ireport")) //$NON-NLS-1$
	...
	public ReportBundle generateReportBundle(TemplateBundle template, Map<String, Object> settings,
		...
		// Create the dataset, by default we set the dataset run to use the report
		// connection...
		...
		// Set the summary and table height and width according to the new value, and
	 ...
	 * 
	 * @param jrContext
	 *            context of the design to check
	 * @param design
	 *            the design to check
	...
	public static List<String> validateJasperDesig(JasperReportsContext jrContext, JasperDesign design) {
		...
		if (tableComponent == null)
			...
			if (table.getColumns().size() > 1 || table.getColumns().size() < 1) {
				...
				if (findStaticTextElement(table, TEXT_MARKER) == null)
					errorsList.add(MessageFormat.format(Messages.TableTemplateEngine_missingStaticText,
							new Object[] { TEXT_MARKER }));
				if (findTextFieldElement(table, FIELD_MARKER) == null)
					errorsList.add(MessageFormat.format(Messages.TableTemplateEngine_missingTextField,
							new Object[] { FIELD_MARKER }));
			}
		}
