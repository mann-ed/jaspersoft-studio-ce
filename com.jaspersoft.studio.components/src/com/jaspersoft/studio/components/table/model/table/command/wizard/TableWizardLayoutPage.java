...
public class TableWizardLayoutPage extends JSSHelpWizardPage {
	...
	/**
	 * Template used to initialize the data of the dialog with the same of the
	 * template, useful for edit
	 */
	...
	/**
	 * String used as id for the button that represent the color used in the
	 * table header cells
	 */
	...
	/**
	 * String used as id for the button that represent the color used in the
	 * table detail cells, checked the row is odd and the attribute to alternate
	 * the rows color is true
	 */
	...
	/**
	 * Listener called when a control get a selection event, cause the
	 * regeneration of the lastGeneratedStyle and the update of the preview
	 */
	...
	/**
	 * When the colors are selected using the schema\variation input method this
	 * method is called to update the every button that represent a color with
	 * the appropriate color
	 */
	private void updateSelectedColor() {
		String colorName = selectionWidget.getSchemaSelected();
		Color color = ColorSchemaGenerator.getColor(colorName);
		ColorSchemaGenerator.SCHEMAS variantKey = selectionWidget
				.getVariantSelectedKey();
		AlfaRGB baseColor = new AlfaRGB(new RGB(color.getRed(),
				color.getGreen(), color.getBlue()), color.getAlpha());
		TableStyle tempStyle = new TableStyle(baseColor, variantKey,
				TableStyle.BorderStyleEnum.FULL,
				AlfaRGB.getFullyOpaque(ColorConstants.white.getRGB()), true);
		selectionWidget.setButtonData(tableHeadrButton,
				tempStyle.getColor(TableStyle.COLOR_TABLE_HEADER));
		selectionWidget.setButtonData(columnHeadrButton,
				tempStyle.getColor(TableStyle.COLOR_COL_HEADER));
		selectionWidget.setButtonData(altDetailButton,
				tempStyle.getColor(TableStyle.COLOR_DETAIL));
		selectionWidget.setButtonData(detailButton,
				tempStyle.getColor(TableStyle.STANDARD_COLOR_DETAIL));
	}
	...
	private void createCellColors(Composite parent) {
		...
		selectionWidget.addButton(tableHeadrButton,
				Messages.TableWizardLayoutPage_tableHeaderLabel,
				temp.getColor(TableStyle.COLOR_TABLE_HEADER));
		selectionWidget.addButton(columnHeadrButton,
				Messages.TableWizardLayoutPage_columnHeaderLabel,
				temp.getColor(TableStyle.COLOR_COL_HEADER));
		selectionWidget.addButton(detailButton,
				Messages.TableWizardLayoutPage_detailLabel,
				temp.getColor(TableStyle.STANDARD_COLOR_DETAIL));
		selectionWidget.addButton(altDetailButton,
				Messages.TableWizardLayoutPage_altDetailLabel,
				temp.getColor(TableStyle.COLOR_DETAIL));
	...
	private void setBorderButtons(BorderStyleEnum loadedStyle) {
		...
		if (borderStyle.equals(BorderStyleEnum.FULL))
			buttonFull.setSelection(true);
		else if (borderStyle.equals(BorderStyleEnum.PARTIAL_VERTICAL))
	...
	/**
	 * Initialize the data if a template to edit was provided, so all the
	 * controls will be initialized with the data read from that template
	 */
	private void setData() {
		if (templateToOpen instanceof TableStyle) {
			...
			selectionWidget.setButtonData(tableHeadrButton,
					templateToOpen.getColor(TableStyle.COLOR_TABLE_HEADER));
			selectionWidget.setButtonData(columnHeadrButton,
					templateToOpen.getColor(TableStyle.COLOR_COL_HEADER));
			selectionWidget.setButtonData(altDetailButton,
					templateToOpen.getColor(TableStyle.COLOR_DETAIL));
			selectionWidget.setButtonData(detailButton,
					templateToOpen.getColor(TableStyle.STANDARD_COLOR_DETAIL));
			// When a template is open for the edit then the colors input method
			// is set to manual by default
			...
			if (titleText != null)
				titleText.setText(cStyle.getDescription());
		}
	}
	 ...
	 * 
	 * @return a TemplateSections class, that contains a series of boolean flags
	 *         used to know which sections of the table are visible.
	 */
	public TableSections getVisibileSections() {
		return new TableSections(isTableHeader, isTableFooter, isColumnHeader,
				isColumnFooter, isGroupHeader, isGroupFooter);
	}
	...
	private void createCellBorders(Composite parent) {
		...
		borderColor.setColor(AlfaRGB.getFullyOpaque(ColorConstants.black
				.getRGB()));
		...
		buttonFull.setImage(Activator.getDefault().getImage(
				"icons/full_borders.png")); //$NON-NLS-1$
		borderStyleButtons.add(buttonFull);

		ToolItem buttonHorizontal1 = new ToolItem(toolBar, SWT.RADIO);
		buttonHorizontal1.setImage(Activator.getDefault().getImage(
				"icons/horizontal_borders.png")); //$NON-NLS-1$
		borderStyleButtons.add(buttonHorizontal1);

		ToolItem buttonHorizontal2 = new ToolItem(toolBar, SWT.RADIO);
		buttonHorizontal2.setImage(Activator.getDefault().getImage(
				"icons/horizontal_borders2.png")); //$NON-NLS-1$
	...
	/**
	 * Create the bottom area of the dialog, where there are the option on the
	 * section that will be created
	 * 
	 * @param parent
	 *            parent of the bottom area
	 */
	...
	public static TableStyle getDefaultStyle() {
		...
		AlfaRGB rgbColor = AlfaRGB.getFullyOpaque(new RGB(color.getRed(), color
				.getGreen(), color.getBlue()));
		return new TableStyle(rgbColor, ColorSchemaGenerator.SCHEMAS.DEFAULT,
				BorderStyleEnum.FULL,
	...
	/**
	 * Return the last generated style for the table, that is the effective one
	 * when the dialog was closed. If the last generated style is null (maybe
	 * because the wizard was finished without reach the last step), the default
	 * one is provided
	 * 
	...
	/**
	 * Called when some property change, rebuild the table style with the actual
	 * state of the control and request the redraw of the preview
	 */
	private void notifyChange() {
		...
		AlfaRGB detail = selectionWidget.getButtonData(detailButton);
		AlfaRGB altDetail = selectionWidget.getButtonData(altDetailButton);
		lastGeneratedStyle = new TableStyle(tableHeader, columnHeader, detail,
				altDetail, borderStyle, borderColor.getColor(),
				alternateColor.getSelection());
		if (titleText != null)
			lastGeneratedStyle.setDescription(titleText.getText());
	...
	/**
	 * Set a template that will be used to initialize (if possible) the control
	 * of the wizard with the value of the template
	 * 
	 * @param template
	 *            template used to initialize the value
	 */
