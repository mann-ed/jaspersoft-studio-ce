...
/**
 * Offers the support methods that using the result of a mondrian or xmla query
 * generate a set of JRDesignFields
 * 
...
public class OlapFieldsProviderSupport {
	...
	public static List<JRDesignField> getFieldsFromResult(JROlapResult result) {
		 ...
		 * <fieldDescription><![CDATA[Data([Measures].[Sales
		 * Amount],?)]]></fieldDescription> </field>
		 * 
		 * We are not mapping.....
		 * 
		 ...
		 * <fieldDescription><![CDATA[Rows[Address][Person](Person
		 * Address)]]></fieldDescription> </field>
		 * 
		 * .....values at different summarization levels
		 * 
		 * <field name="StoreStateUnitSales" class="java.lang.Number">
		 * <fieldDescription><![CDATA[Data(Rows[Store][Store State], Rows[Time][Year],
		 * Rows[Customers][(All)]) ([Measures].[Unit Sales],?)]]></fieldDescription>
		 * </field>
		 * 
		 * .........Members: only in Mondrian results
		 * 
		 * <field name="AddressMember" class="mondrian.olap.Member">
		 * <fieldDescription><![CDATA[Rows[Address]]]></fieldDescription> </field>
		 * 
		 */
		...
		for (JROlapResultAxis axis : result.getAxes()) {
			...
			for (JROlapHierarchy hier : axis.getHierarchiesOnAxis()) {
				...
				for (JROlapHierarchyLevel level : hier.getLevels()) {
					if (level != null) {
						if (level.getName().equalsIgnoreCase("MeasuresLevel")) {
							...
							String rowsExpression = prefix + makeOlapExpression(hier.getDimensionName()) + "["
									+ level.getName() + "]";

							addField(makeJRFieldName(hier.getDimensionName() + level.getName()), rowsExpression,
				...
				/*
				 * Pick up the measure names. There should always be a measuresLevel.
				 */
				if (foundMeasuresLevel) {
					for (int i = 0; i < axis.getTupleCount(); i++) {
						...
						for (int j = 0; j < memberTuple.getMembers().length; j++) {
							...
							if (isMeasureMember(member.getUniqueName())
		...
		/*
		 * This assumes measures will be only on only rows and columns. will fail for
		 * pages, chapters and sections
		 */
		for (String measureName : measureNames) {
			addField(makeJRFieldName(measureName),
					generateMeasureAxisExpression(measureLevelAxisIndex, axisCount - 1, measureName),
					"java.lang.Number", fields);
		}
