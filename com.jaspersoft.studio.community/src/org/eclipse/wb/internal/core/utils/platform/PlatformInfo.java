...
public final class PlatformInfo {
	...
	/**
	 * The development environment's NL (not null, not empty, no leading or
	 * trailing spaces)
	 */
	...
	/**
	 * The build identifier for Eclipse (e.g. "v20061503") or <code>null</code>
	 * if not initialized yet by {@link #getEclipseBuildId()}. For Eclipse 3.2,
	 * this is the same as the Eclipse version qualifier segment. For older
	 * versions of Eclipse, this is read from the JDT plugin about.properties
	 * file.
	 */
	private static String eclipseBuildId;
	/**
	 * The name for this particular Eclipse build (e.g. "M6", "M7", "RC1", "",
	 * etc) or <code>null</code> if not initialized yet by
	 * {@link #getEclipseBuildName()}. This may contain the same value as
	 * {@link #getEclipseBuildId()} if the build is not a milestone, release
	 * candidate, or GA.
	 */
	private static String eclipseBuildName;
	/**
	 * The build date for this particular instance of Eclipse as derived from
	 * {@link #getEclipseBuildId()} or <code>null</code> if not initialized yet
	 * by {@link #getEclipseBuildDate()}.
	 */
	 ...
	 * 
	 * @return the full name (not null, not empty, no leading or trailing
	 *         spaces)
	 */
	 ...
	 * 
	 * @return the version string (not null, not empty, no leading or trailing
	 *         spaces)
	 */
	 ...
	 * 
	 * @return the national language being used (not null, not empty, no leading
	 *         or trailing spaces)
	 */
	...
	/**
	 * Answer the version of Eclipse upon which the development environment is
	 * based.
	 * 
	...
	public static Version getEclipseVersion() {
		if (eclipseVersion == null) {
			try {
				...
				// Use org.eclipse.jdt.ui to tell the difference between E-3.5
				// and E-3.6 M#
				...
				if (eclipseVersion.getQualifier().length() == 0) {
					eclipseVersion = new Version(eclipseVersion.getMajor(), eclipseVersion.getMinor(),
							eclipseVersion.getMicro(), "v" + readBuildId("org.eclipse.jdt", "about.mappings"));
				}
			} catch (Exception e) {
				// Logger may not be open, so cannot log exception
			}
	...
	/**
	 * Answer the build identifier for Eclipse (e.g. "200408122000"). This is
	 * read from the JDT plugin about.properties file.
	 * 
	 * @return the build id or {@link #UNKNOWN_BUILD_ID} if it cannot be
	 *         determined
	 */
	...
	/**
	 * Answer the name for this particular Eclipse build (e.g. "M6", "M7",
	 * "RC1", "", etc). This may return the same value as
	 * {@link #getEclipseBuildId()} if the build is not a milestone, release
	 * candidate, or GA.
	 * 
	 * @return the build name (not <code>null</code>, but may be empty if it is
	 *         a GA)
	 */
	...
	/**
	 * Answer the build date for this particular instance of Eclipse as derived
	 * from {@link #getEclipseBuildId()} or Jan 1, 2003 if it cannot be
	 * determined.
	 * 
	 ...
	 * @return the build identifier or <code>null</code> if it cannot be
	 *         determined
	 */
