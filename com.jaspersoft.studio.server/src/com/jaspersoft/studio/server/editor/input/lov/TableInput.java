...
public class TableInput implements IInput {
	...
	public void createControl(Composite parent, int style) {
		table = new Table(parent, style | SWT.V_SCROLL | SWT.H_SCROLL | SWT.BORDER | SWT.FULL_SELECTION);
				...
				if (value instanceof List) {
					...
					for (TableItem ti : table.getItems()) {
						if (lst.contains(ti.getData())) {
							titems.add(ti);
							if (ct == ResourceDescriptor.IC_TYPE_MULTI_SELECT_LIST_OF_VALUES_CHECKBOX)
								ti.setChecked(true);
						}
					}
