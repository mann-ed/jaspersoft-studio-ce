...
/**
 * Modified version of the {@link TitleAreaDialog} that truncate the text in the message 
 * area when longer than two lines, placing "..." after the last word and the full
 * message in the tooltip of the message area.
 * This is a full refactor of the {@link TitleAreaDialog} since the message area
 * and all the method to access it were private in the original version
 * 
 ...
 */
public class JSSTitleAreaDialog extends TrayDialog {
	/**
	...
	protected boolean isMessageTruncated = false;
	 ...
	 */
	public JSSTitleAreaDialog(Shell parentShell) {
		super(parentShell);
	}
	...
	protected Control createTitleArea(Composite parent) {
		...
		messageLabel.addControlListener(new ControlAdapter() {
			...
			public void controlResized(ControlEvent e) {
				updateMessage(showingError && errorMessage != null ? errorMessage : message, messageLabel);
			}
		});
	...
	public void setErrorMessage(String newErrorMessage) {
		...
		if (errorMessage == null) {
			...
			if (message == null) // this should probably never happen since
				...
				message = ""; //$NON-NLS-1$
			updateMessage(message, messageLabel);
			...
			updateMessage(displayedErrorMessage, messageLabel);
			if (!showingError) {
	...
	private void layoutForNewMessage(boolean forceLayout) {
		...
		if (messageLabelClipped || isMessageTruncated) {
	...
	protected String getToolTipText() {
		return showingError && errorMessage != null ? errorMessage : message;
	}
	...
	private void showMessage(String newMessage, Image newImage) {
		...
		if (!showingError) {
			...
			updateMessage(shownMessage, messageLabel);
			messageImageLabel.setImage(messageImage);
			setImageLabelVisible(messageImage != null);
			layoutForNewMessage(false);
		}
	...
	protected String getTruncatedText(String newMessage, Text messageLabel) {
		Rectangle bounds = messageLabel.getBounds();
		GC gc = new GC(messageLabel);
		try
		{
		    gc.setFont(messageLabel.getFont());
		    FontMetrics fm = gc.getFontMetrics();
		   	int charsPerLine = bounds.width / fm.getAverageCharWidth();
	   		int totalChars = (charsPerLine * 2) - 3;
	   		if (totalChars > 0 && totalChars < newMessage.length()-1) {
	   			isMessageTruncated = true;
	   			String truncatedMessage = newMessage.substring(0, totalChars);
	   			char breakPoint = newMessage.charAt(totalChars);
	   			 if (breakPoint != ' ') {
	   				int lastSpace = truncatedMessage.lastIndexOf(' ');
	   				if (lastSpace != -1) {
	   					truncatedMessage = truncatedMessage.substring(0, lastSpace);
	   				}
	   			}
	 ...
	 *            the message to use
	 */
	protected void updateMessage(String newText, Text messageLabel) {
		String oldMessage = messageLabel.getText();
		String newMessage = getTruncatedText(newText, messageLabel);
		if (newMessage.equals(oldMessage)) return;
