...
/**
 * A dialog that can store the size and position when it is closed, to be opened
 * in the same position the next time it is opened
 * 
...
public class PersistentLocationFormDialog extends FormDialog {
	...
	protected PersistentLocationFormDialog(Shell parentShell) {
		super(parentShell);
	}
	 ...
	 * even if the dialog is palced into a not valid monitor
	 */
	@Override
	 ...
	 *            position to check
	 * @return true if the position is trully inside an existing monitor, false
	 *         otherwise
	 */
	...
	/**
	 * Return the point where the dialog should be placed to be ceneterd in the
	...
	/**
	 * Set if the dialog should store size and position when it is closed, they
	 * will be reused the next time it is opened. By default this is true
	 * 
	 * @param value
	 *            true if the settings should be stored, false otherwise
	 */
