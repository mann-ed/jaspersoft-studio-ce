...
public class SWTResourceManager {
	...
	private static Map<RGBA, Color> m_rgba_colorMap = new HashMap<RGBA, Color>();
	...
	public static Color getColor(RGBA rgba) {
		Color color = m_rgba_colorMap.get(rgba);
		if (color == null) {
			Display display = Display.getCurrent();
			color = new Color(display, rgba);
			m_rgba_colorMap.put(rgba, color);
		}
		return color;
	}		
	
	/**
	 * Return all the stored colors
	 */
	public static Set<RGB> getStoredColors(){
		return m_colorMap.keySet();
	}
	...
	public static void disposeColors() {
		...
		for (Color color : m_rgba_colorMap.values()) {
			color.dispose();
		}
		m_colorMap.clear();
		m_rgba_colorMap.clear();
	}
	...
	public static Image decorateImage(final Image baseImage, final Image decorator, final int corner) {
		if (corner <= 0 || corner >= LAST_CORNER_KEY) {
			throw new IllegalArgumentException(Messages.SWTResourceManager_WrongCornerDecoration);
		}
