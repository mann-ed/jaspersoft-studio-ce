...
public class ResourceManager extends SWTResourceManager {
	...
	@SuppressWarnings("unchecked")
	private static Map<ImageDescriptor, Map<ImageDescriptor, ImageDescriptor>>[] m_decoratedImageDescriptorMap = new Map[LAST_CORNER_KEY];
	/**
	...
	public static Image decorateImage(final Image baseImage, final Image decorator, final int corner) {
		if (corner <= 0 || corner >= LAST_CORNER_KEY) {
			throw new IllegalArgumentException(Messages.ResourceManager_WrongCornerDecoration);
		}
	...
	public static ImageDescriptor decorateImage(final ImageDescriptor baseImage, final ImageDescriptor decorator, final int corner) {
		if (corner <= 0 || corner >= LAST_CORNER_KEY) {
			throw new IllegalArgumentException(Messages.ResourceManager_WrongCornerDecoration);
		}
	...
	//
	////////////////////////////////////////////////////////////////////////////
	private static Map<String, Cursor> m_descriptorCursorsMap = new HashMap<String, Cursor>();
	...
	public static void disposeCursors() {
		SWTResourceManager.disposeImages();
		{
			for (Iterator<Cursor> I = m_descriptorCursorsMap.values().iterator(); I.hasNext();) {
				I.next().dispose();
			}
			m_descriptorCursorsMap.clear();
		}
	}
	
	/**
	 * Returns an {@link Cursor} with an image stored in the file at the specified path relative to the specified
	 * class.
	 * 
	 * @param clazz the {@link Class} relative to which to find the image descriptor for the cursor.
	 * @param path the path to the image file.
	 * @return the {@link Cursor} for the image stored in the file at the specified path.
	 */
	public static Cursor getCursor(Class<?> clazz, String imagePath) {
		String key = clazz.getName()+imagePath;
		Cursor cursor = m_descriptorCursorsMap.get(key);
		if (cursor == null){
			ImageDescriptor descriptor = getImageDescriptor(clazz, imagePath);
			cursor = new Cursor(UIUtils.getDisplay(), descriptor.getImageData(), 0, 0);
			m_descriptorCursorsMap.put(key, cursor);
		}
		return cursor;
	}
	 ...
	 * @param clazz the {@link Class} relative to which to find the image descriptor for the cursor.
	 * @param path the path to the image file.
	 * @param maskName path of the mask data for the cursor (or null)
	 * @return the {@link Cursor} for the image stored in the file at the specified path.
	 */
	public static Cursor getCursor(Class<?> clazz, String sourceName, String maskName, int hotspotX, int hotspotY) {
		String key = clazz.getName() + sourceName + Misc.nvl(maskName) + hotspotX + hotspotY;
		Cursor cursor = m_descriptorCursorsMap.get(key);
		if (cursor == null){
			ImageDescriptor src = ImageDescriptor.createFromFile(clazz, sourceName);
			ImageDescriptor mask = ImageDescriptor.createFromFile(clazz, maskName);
			cursor = new Cursor(UIUtils.getDisplay(), src.getImageData(), mask.getImageData(), hotspotX, hotspotY);
			m_descriptorCursorsMap.put(key, cursor);
		}
		return cursor;
	}
