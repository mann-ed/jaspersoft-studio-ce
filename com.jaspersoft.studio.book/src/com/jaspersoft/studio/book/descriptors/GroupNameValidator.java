...
/**
 * Validator to check if a name for a group is valid. It is valid essentially if
 * it is unique. If it is not then an error message is returned
 * 
...
public class GroupNameValidator extends AbstractJSSCellEditorValidator {
	...
	/**
	 * The object must be the new name for the group, and using the target check if
	 * there are other variables with the same name, under the same dataset
	 */
	@Override
	public String isValid(Object value) {
		MGroupReportPartContainer section = (MGroupReportPartContainer) getTarget();
		if (section.getJrgroup() == null)
			return null;
		if (value == null)
			return null;
		...
		if (d != null) {
			...
			if (groupVar != null) {
				String message = MessageFormat.format(Messages.GroupSection_SameVariableNameErrorMsg,
						new Object[] { value + "_COUNT", value });
				return message;
			}
		}
		return null;
